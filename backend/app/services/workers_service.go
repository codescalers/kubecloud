...
func NewWorkersService(
	...
	invoiceCompanyData internal.InvoiceCompanyData, systemMnemonic, currency string,
...
func (svc WorkerService) CreateUserInvoice(user models.User) error {
	...
	for _, record := range records {
		billReports, err := internal.ListContractBillReportsPerMonth(svc.graphql, record.ContractID, now)
		if err != nil {
			return err
		}
		...
		var totalHours int
		cancellationDate, err := internal.GetRentContractCancellationDate(svc.firesquidClient, record.ContractID)
	...
	file, err := internal.CreateInvoicePDF(invoice, user, svc.invoiceCompanyData)
	if err != nil {
		return err
	}
...
func (svc WorkerService) AsyncTrackClusterHealth(cluster models.Cluster) error {
	wf, err := svc.ewfEngine.NewWorkflow(constants.WorkflowTrackClusterHealth)
	if err != nil {
	...
	svc.ewfEngine.RunAsync(svc.ctx, wf)
	return nil
}
