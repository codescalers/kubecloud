...
func (svc *NodeService) AsyncReserveNode(userID int, userMnemonic string, nodeID uint32) (string, error) {
	wf, err := svc.ewfEngine.NewWorkflow(internal.WorkflowReserveNode)
	if err != nil {
		return "", err
	}
	...
	wf.State = map[string]interface{}{
		...
		"node_id":       nodeID,
		"target_status": internal.NodeRented,
	}
...
func (svc *NodeService) AsyncUnreserveNode(userID int, userMnemonic string, contractID uint64, nodeID uint32) (string, error) {
	wf, err := svc.ewfEngine.NewWorkflow(internal.WorkflowUnreserveNode)
	if err != nil {
		return "", err
	}
	...
	wf.State = map[string]interface{}{
		...
		"node_id":       nodeID,
		"target_status": internal.NodeRentable,
	}
