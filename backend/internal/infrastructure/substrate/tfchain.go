...
func (c *TFChainClient) GetTFTBillingRateAt(block uint64) (float64, error) {
	rate, err := c.subConn.GetTFTBillingRateAt(block)
	if err != nil {
		return 0, err
	}
...
func (c *TFChainClient) GetPricingPolicy(id uint32) (substrate.PricingPolicy, error) {
	return c.subConn.GetPricingPolicy(id)
}

func (c *TFChainClient) GetCurrentHeight() (uint32, error) {
	return c.subConn.GetCurrentHeight()
}
...
func (c *TFChainClient) activateAccount(substrateAccountID string) error {
	activationServiceURL := grid.ActivationServiceURLs[c.network]
